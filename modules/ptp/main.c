...
// number of timestamps in "external timstamp" fifo
#define RA_PTP_EXTTS_TS_CNT		0x0050
...
static void ra_ptp_ior_rep(struct ra_ptp_priv *priv, off_t reg,
			   void *dst, size_t len)
{
	ioread32_rep(priv->regs + reg, dst, len);
}
...
static int ra_ptp_set_per_out(struct ra_ptp_priv *priv, int ns)
{
	...
	if (ns > 0)
		dev_info(dev, "Periodic output activated with interval of %dns\n",
			 ns);
	else
...
static int ra_ptp_gettime(struct ptp_clock_info *ptp, struct timespec64 *ts)
{
	...
	ret = read_poll_timeout(ra_ptp_ior, val,
				val & RA_PTP_STATUS_READ_CLOCK_VALID,
				10, 100, false, priv, RA_PTP_STATUS);
	if (ret < 0)
		return ret;
			  ...
			  const struct timespec64 *ts)
{
	...
	spin_lock_irqsave(&priv->lock, flags);
	ra_ptp_iow(priv, RA_PTP_READ_TIME_SECONDS_H, ts->tv_sec >> 32);
...
static int ra_ptp_adjtime(struct ptp_clock_info *ptp, s64 delta)
{
	...
	if (delta <= RA_PTP_ADJ_TIME_MAX_OFFSET) {
		...
		dev_dbg(dev, "%s(): register value: %02x\n", __func__, val);
		...
		dev_dbg(dev, "%s(): PTP hw clock adjust: %lldns\n",
			__func__, delta);
			 ...
			 int on)
{
	...
	case PTP_CLK_REQ_EXTTS:
		...
		if (on) {
			...
			dev_dbg(dev, "%s: disable EXTTS\n", __func__);
			ra_ptp_write_mask(priv, RA_PTP_IRQ_DISABLE,
					  RA_PTP_IRQ_EXTTS, RA_PTP_IRQ_EXTTS);
			ra_ptp_write_mask(priv, RA_PTP_EXTTS_MODE,
					  RA_PTP_EXTTS_MODE_ENABLE_EXTTS, 0);
		}
	...
	case PTP_CLK_REQ_PPS:
		if (on) {
			dev_dbg(dev, "%s(): enable PPS\n", __func__);
			ra_ptp_write_mask(priv, RA_PTP_IRQ_DISABLE,
					  RA_PTP_IRQ_PPS, 0);
		} else {
			dev_dbg(dev, "%s(): disable PPS\n", __func__);
			ra_ptp_write_mask(priv, RA_PTP_IRQ_DISABLE,
					  RA_PTP_IRQ_PPS, RA_PTP_IRQ_PPS);
		}
...
static irqreturn_t ra_ptp_irqhandler(int irq, void *devid)
{
	...
	irqs = ra_ptp_ior(priv, RA_PTP_IRQS);
	mask = ra_ptp_ior(priv, RA_PTP_IRQ_DISABLE);

	if (unlikely((irqs & ~mask) == 0)) {
		dev_dbg(dev, "%s: invalid IRQ received: 0x%04X (mask 0x%04X)\n",
			__func__, irqs, mask);
	...
	if (irqs & RA_PTP_IRQ_EXTTS) {
		...
		dev_dbg(dev, "%s(): handle EXTTS IRQ\n", __func__);
		...
		while (ts_cnt--) {
			...
			if (ctr) {
				...
				ra_ptp_ior_rep(priv, RA_PTP_EXTTS_DATA,
					       &extts.seconds,
					       sizeof(extts.seconds));

				// FIXME: hier fehlen die extts.seconds_hi
				event.timestamp = (uint64_t)extts.seconds * NSEC_PER_SEC;
				event.timestamp += extts.nanoseconds;
	...
	// this should add ptp clock timestamp to the ptp clock pps event
	if (irqs & RA_PTP_IRQ_PPS) {
		struct timespec64 ts;

		dev_dbg(dev, "%s: handle PPS IRQ\n", __func__);
		...
		if (ra_ptp_gettime(&priv->ptp_clock_info, &ts) == 0) {
				  ...
				  char *buf)
{
	...
	return sysfs_emit(buf, "0x%x 0x%x\n",
...
static int ra_ptp_drv_probe(struct platform_device *pdev)
{
	...
	strlcpy(priv->ptp_clock_info.name, "ravenna_ptp",
		sizeof(priv->ptp_clock_info.name));
